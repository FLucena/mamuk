...
// Type for user data without Document properties
type UserData = Omit<IUser, keyof Document | '_id'> & { _id?: mongoose.Types.ObjectId };

// Generate random users
async function generateUsers(count: number): Promise<UserData[]> {
  ...
  for (let i = 0; i < count - 4; i++) {
    ...
    if (faker.datatype.boolean({ probability: 0.6 }) && coaches.length > 0) {
